...
public class FileHandler {
	...
	// 저장 경로
	private final String UP_DIR = "D:\\_mySpringProject\\_java\\_fileUpload";
	
	public List<FileVO> uploadFiles(MultipartFile[] files){
		
		List<FileVO> flist = new ArrayList<>();
		// FileVO 생성 + 파일 저장 + 썸네이 저장
		...
		// D:\\_mySpringProject\\_java\\_fileUpload\\2024\\11\\01 
		File folders = new File(UP_DIR, today);
		
		// 폴더 생성(있으면 생성 안함) : mkdir (1개만 생성) mkdirs(하위촐더까지 생성)
		if(!folders.exists()) {
			folders.mkdirs();
		}
